...
SELECT sexo, count(sexo) from funcionarios
GROUP BY sexo;

/* colocando o nome da coluna */

SELECT sexo, count(sexo) as "Quantidade" from funcionarios
GROUP BY sexo;
...
/* exibindo o maximo e minimo juntos */

SELECT MIN(salario) as "minimo salario", max(salario) as "maximo salario" 
from funcionarios;

/* exibindo o maximo e minimo juntos de cada departamento */

SELECT departamento, MIN(salario) as "minimo salario", max(salario) as "maximo salario" 
from funcionarios
group by departamento;

/* exibindo o maximo e minimo juntos por sexo */

SELECT sexo, MIN(salario) as "minimo salario", max(salario) as "maximo salario" 
from funcionarios
group by sexo;
...
/* por departamento, qual total e media para cada funcionario? */
...
Evitem redundancias, consequentemente poupam espaço em disco.
Consomem muito processamento em função de Joins. Queries lentas

Data Science e B.I. -> Focam em agregações e performance. Não se
Preocupam com espaço em disco. Em B.I., modelagem minima (DW)
em Data Science, preferencialmente modelagem colunar */

/* o Servidor e maquinas geraram um arquivo de log CSV<
...
SELECT MAQUINA, AVG(QTD) AS MEDIA FROM MAQUINAS
group by MAQUINA
ORDER BY 2 DESC;

/* ARREDONDANDO */

SELECT MAQUINA, ROUND(AVG(QTD), 2) AS MEDIA FROM MAQUINAS
group by MAQUINA
ORDER BY 2 DESC;

/* QUAL É A MODA DAS QUANTIDADES? */

SELECT MAQUINA, QTD, COUNT(*) AS MODA FROM MAQUINAS
GROUP BY MAQUINA, QTD
ORDER BY 3 DESC;

/* QUAL É A MODA DAS QUANTIDADES DE CADA MAQUINA? */
...
/* QUAL É O MAXIMO E MINIMO E AMPLITUDEDE CADA MAQUINA? */
...
SELECT 
	...
	ROUND((STDDEV_POP(QTD) / AVG(QTD)) * 100, 2) AS "CONFICIENTE DE VARIACAO"
FROM MAQUINAS
GROUP BY MAQUINA
ORDER BY 1;

DELETE FROM MAQUINAS WHERE DIA IN (11,12);
...
/* EXPORTACAO DE TABELA PARA MODELO COLUNAR */
...
SELECT * FROM FILME F
JOIN GENERO G ON F.ID_GENERO = G.IDGENERO;
...
JOIN FILME F ON L.ID_FILME = F.IDFILME;
...
SELECT L.IDLOCACAO, F.NOME AS FILME, G.NOME AS GENERO, 
...
SELECT * FROM LOCACAO ORDER BY IDLOCACAO DESC;
SELECT * FROM RELATORIO_LOCADORA ORDER BY IDLOCACAO DESC;

/* SINCRONILIZANDO COM OS REGISTROS DELETADOS */
...
/* 1 - QUAL A MODA DOS SALARIOS. A MODA DOS SALARIOS DIZ ALGO RELEVANTE? */
/* 
RESPOSTA: HÁ UMA CHANCE MAIOR DE TER ESSES SALÁRIOS, SENDO OS:
47281	2
52956	2
66063	2
78157	2
...
144724	2
*/
SELECT SALARIO, COUNT(*) AS MODA FROM FUNCIONARIOS
GROUP BY SALARIO
ORDER BY 2 DESC, 1;

/* 2 - QUAL A MODA DEPARTAMENTAL = QUAL O DEPARTAMENTO QUE MAIS EMPREGA? */
-- RESPOSTA: BELEZA E ROUPAS

SELECT DEPARTAMENTO, COUNT(*) AS MODA FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
ORDER BY 2 DESC, 1;

/* 3 - QUAL O DESVIO PADRAO DE CADA DEPARTAMENTO? */
...
/* 4 - CALCULE A MEDIANA SALARIAL DE TODO O SET DE DADOS */
-- RESPOSTA: 96278.50

SELECT ROUND(MEDIAN(SALARIO),2) AS MEDIANA
FROM FUNCIONARIOS;

/* 5 - QUAL A AMPLITUDE DE TODOS OS SALARIOS? */

SELECT 
	MAX(SALARIO) AS MAXIMO, 
	MIN(SALARIO) AS MINIMO, 
	MAX(SALARIO) - MIN(SALARIO) AS "AMPLITUTE"
FROM FUNCIONARIOS;

/* 6 - CALCULE AS PRINCIPAIS MEDIDAS ESTATISTICAS POR DEPARTAMENTO */
...
SELECT 
	...
	ROUND((STDDEV_POP(SALARIO) / AVG(SALARIO)) * 100, 2) AS "CONFICIENTE DE VARIACAO",
	MODE() WITHIN GROUP(ORDER BY SALARIO) AS "MODA"
FROM FUNCIONARIOS
GROUP BY DEPARTAMENTO
ORDER BY 1;

/* 7 - QUAL DEPARTAMENTO TE DA UMA MAIOR PROPABILIDADE DE GANHAR MAIS? --OUTDOOR */
-- RESPOSTA: USEI O MEDIANA PARA DEFINIR QUAL O DEPARTAMENTO PAGA MELHOR
...
/* FILTROS BASEADOS EM MULTIPLOS TIPOS E COLUNAS - CONSIDERAR AND E OR */
...
/* FILTROS BASEADOS EM UNICO TIPO E COLUNA - ATENCAO PARA A REGRA
DO AND E OR - EM RELACIONAMENTO 1 PARA 1, O FILTRO AND DA MESMA COLUNA 
SEMPRE DARA FALSO */
...
(SELECT COUNT(*) FROM FUNCIONARIOS WHERE SEXO = 'Masculino') AS "MASCULINO"
FROM FUNCIONARIOS;
...
COUNT(*) FILTER (WHERE SEXO = 'Masculino') AS "MASCULINO",
COUNT(*) FILTER (WHERE DEPARTAMENTO = 'Books') AS "Books",
COUNT(*) FILTER (WHERE SALARIO > 140000) AS "SALARIO140K"
FROM FUNCIONARIOS;
...
SELECT UPPER(CARGO || ' - ' || DEPARTAMENTO) AS CARGO
FROM FUNCIONARIOS;
...
/* DESAFIO: CRIAR UMA COLUNA DO LADO DA COLUNA CARGO
PARA TRAZER SE ELA É ASSISTENTE OU NÃO */

/* O JEITO QUE FIZ */
SELECT NOME, CARGO,
CASE
	WHEN UPPER(CARGO) LIKE '%ASSISTANT%' THEN 'SIM'
	ELSE 'NÃO'
END AS "É ASSISTENTE"
FROM FUNCIONARIOS
ORDER BY 3;
...
SELECT NOME, (CARGO LIKE '%Assistant%') "ASSISTENTE?"
FROM FUNCIONARIOS;
